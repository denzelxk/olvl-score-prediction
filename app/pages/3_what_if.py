...
with st.sidebar:
    ...
    use_uploaded = ("predictions_df" in st.session_state
                    and st.checkbox("Use uploaded class", value=True))

def _defaults():
    return dict(age=15.0,hours_per_week=10.0,attendance_rate=93.0,
...
if use_uploaded:
    ...
    def g(c, d):                                          # ← FIX: was a lambda
        if c not in row.index:
            return d
        val = row[c]
        if c == "tuition":                                # ← handles "Yes"/"No"/"Y"/"N"
            return _TUITION_MAP.get(str(val), int(float(val)) if str(val).lstrip("-").isdigit() else d)
        try:
            return float(val)
        except (ValueError, TypeError):
            return d
    base = dict(age=g("age",15),hours_per_week=g("hours_per_week",10),
    ...
    base = _defaults(); baseline_score = None
...
def _df(f):
    return pd.DataFrame([{"age":f["age"],"hours_per_week":f["hours_per_week"],
        ...
        "gender":f["gender"],"mode_of_transport":f["mode_of_transport"],"tuition":int(f["tuition"])}])

try:
    pipeline = load_model(model_key)
    if baseline_score is None: baseline_score = float(pipeline.predict(_df(base))[0])
    ...
    changes = []
    if new_tval  != base["tuition"]:               changes.append(f"Tuition: {'added ✅' if new_tval else 'removed'}")
    if abs(new_hours  - base["hours_per_week"])>0.1: changes.append(f"Hours: {base['hours_per_week']:.0f}→{new_hours:.0f}")
    if abs(new_attend - base["attendance_rate"])>0.1: changes.append(f"Attendance: {base['attendance_rate']:.1f}→{new_attend:.1f}%")
    if new_cca != base["CCA"]:                    changes.append(f"CCA: {base['CCA']}→{new_cca}")
    if changes: st.info("**Applied:** " + " | ".join(changes))
